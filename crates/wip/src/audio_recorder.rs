pub(crate) use crate::{
    app::{MAX_RECORDING_SECONDS, TARGET_SAMPLE_RATE},
...
impl AudioRecorder {
    pub fn new() -> WipResult<Self> {
        let host = cpal::default_host();
        let device = host
            .default_input_device()
            .ok_or_else(|| WipError::audio_device("no default input device is available"))?;
        ...
        if channels == 0 {
            return Err(WipError::audio_device(
        ...
        stream
            .play()
            .map_err(|error| WipError::audio_stream(error.to_string()))?;
    ...
    pub fn start(&self) -> WipResult<()> {
        let mut state = self
            .state
            .lock()
            .map_err(|_| WipError::recorder_state("recording state lock is poisoned"))?;
        state.samples.clear();
    ...
    pub fn stop(&self) -> WipResult<AudioRecording> {
        let mut state = self
            .state
            .lock()
            .map_err(|_| WipError::recorder_state("recording state lock is poisoned"))?;
    ...
    pub fn take_stream_error(&self) -> WipResult<Option<String>> {
        let mut state = self
            .state
            .lock()
            .map_err(|_| WipError::recorder_state("recording state lock is poisoned"))?;
...
fn select_input_config(device: &cpal::Device) -> WipResult<cpal::SupportedStreamConfig> {
    let target_rate = SampleRate(TARGET_SAMPLE_RATE);
    let supported_configs = device
        .supported_input_configs()
        .map_err(|error| WipError::audio_device(error.to_string()))?;
...
fn build_stream(
    ...
    match sample_format {
        ...
        unsupported => Err(WipError::audio_device(format!(
